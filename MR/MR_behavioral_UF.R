# ===== Two-sample MR: Behavioral syndromes -> Uterine Fibroids =====
library(data.table)
library(TwoSampleMR)
library(ieugwasr)
library(tibble)
...
# ---- Paths & output dir ----
exposure_path <- "/gpfs/scratch/asgelz01/MR/behavioral_hg19.txt.gz"  
outcome_path  <- "/gpfs/scratch/asgelz01/MR/UF_hg19_new.txt"         

ts <- format(Sys.time(), "%Y%m%d_%H%M%S")
outdir <- file.path("/gpfs/scratch/asgelz01/MR/results", paste0("behavioral_to_UF_8_", ts))
dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

logf <- file.path(outdir, "log.txt")
cat(sprintf("[%s] Start. Writing to: %s\n", Sys.time(), outdir), file = logf, append = TRUE)

# ---- Read exposure (Behavioral syndromes) ----
# File columns include: chromosome, base_pair_location, effect_allele, other_allele,
# beta, standard_error, effect_allele_frequency, p_value, SNP, N, ...
...
exposure_dat$exposure <- "Behavioral syndromes"
...
outcome_dat$outcome <- "Uterine fibroids"
...
cat(sprintf("[%s] Done. Results in %s\n", Sys.time(), outdir), file = logf, append = TRUE)# ---- Paths & output dir ----
exposure_path <- "/gpfs/scratch/asgelz01/MR/behavioral_hg19.txt.gz"  # Behavioral is now EXPOSURE
outcome_path  <- "/gpfs/scratch/asgelz01/MR/UF_hg19_new.txt"         # UF is now OUTCOME
