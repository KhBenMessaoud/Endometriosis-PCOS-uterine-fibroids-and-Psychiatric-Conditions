# ===== Two-sample MR: Anxiety Disorders -> Endometriosis =====
library(data.table)
library(TwoSampleMR)
library(ieugwasr)
library(tibble)
...
# ---- Paths & output dir ----
exposure_path <- "/gpfs/scratch/asgelz01/MR/anxiety_buildGRCh37.tsv"  
outcome_path  <- "/gpfs/scratch/asgelz01/MR/EM_clean.txt.gz"         

ts <- format(Sys.time(), "%Y%m%d_%H%M%S")
outdir <- file.path("/gpfs/scratch/asgelz01/MR/results", paste0("anxiety_to_EM_6_", ts))
dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

logf <- file.path(outdir, "log.txt")
cat(sprintf("[%s] Start. Writing to: %s\n", Sys.time(), outdir), file = logf, append = TRUE)

# ---- Prepare outcome file (Endometriosis) by de-duplicating rsids ----
EM <- fread(outcome_path)
setnames(EM, gsub("^#", "", names(EM)))

n_before <- nrow(EM)
dup_before <- sum(duplicated(EM$rsids), na.rm = TRUE)

EM2 <- EM[order(pval, sebeta)][!duplicated(rsids)]

n_after <- nrow(EM2)
dup_after <- sum(duplicated(EM2$rsids), na.rm = TRUE)

dedup_path <- file.path(outdir, "EM_dedup.tsv")
fwrite(EM2, dedup_path, sep = "\t")
cat(sprintf("[%s] EM outcome rows before: %d, duplicated rs_id: %d; after: %d, duplicated rs_id: %d\n",
            Sys.time(), n_before, dup_before, n_after, dup_after),
    file = logf, append = TRUE)

# ---- Read exposure (Anxiety disorders) ----
# If your anxiety file has an rsID column, switch snp_col="variant_id" -> that rsID column name.
...
exposure_dat$exposure <- "Anxiety disorders"
head(exposure_dat)

# ---- LD clumping (on Anxiety instruments) ----
